---
name: gbp-optimizer
description: When the user wants to optimize a Google Business Profile listing for local search visibility. Also use when the user mentions "Google Business Profile," "GBP optimization," "Google Maps ranking," or "local pack visibility." For NAP consistency across other directories, use local-citation-audit. For multi-location businesses, use multi-location-seo.
metadata:
  version: 1.0.0
---
...
Single-location GBP optimization. For NAP consistency across third-party directories, see **local-citation-audit**. For businesses managing many locations, see **multi-location-seo**.
...
6. Monitor engagement metrics (views, calls, direction requests) as behavioral signals, not just position tracking, since Google increasingly weighs real-world engagement.

## Related Skills

- **local-citation-audit**: NAP consistency this profile's prominence signal depends on
- **multi-location-seo**: Managing this optimization across many locations
- **review-response-writer**: Drafting the owner responses this skill recommends
- **local-business-llm-visibility** (geo): AI-recommendation layer built on top of a well-optimized GBP
