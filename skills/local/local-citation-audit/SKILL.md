...
description: When the user wants to audit NAP (name, address, phone) consistency across online directories and citation sources. Also use when the user mentions "local citation audit," "NAP consistency," or "directory listings audit." For the primary Google Business Profile itself, use gbp-optimizer.
metadata:
  version: 1.0.0
---
...
Third-party directory/citation consistency. Not the primary GBP listing itself (see **gbp-optimizer**).
...
1. Establish the canonical NAP format to be used everywhere (matching what's on the GBP listing).
2. Search for existing citations across major directories and aggregators.
3. Flag every instance of a name, address, or phone number variant from the canonical format.
4. Prioritize fixing high-authority directories first, then work down to smaller ones.
5. Search specifically for duplicate or stale listings tied to a previous address, name, or phone number, and correct or remove them.
6. Re-audit periodically, since third-party directories can independently alter or duplicate data without the business's involvement.

## Related Skills

- **gbp-optimizer**: The primary listing this consistency check reinforces
- **multi-location-seo**: Citation management complexity multiplies with each additional location
