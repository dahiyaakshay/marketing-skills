...
description: When the user wants to write or optimize a page's title tag. Also use when the user mentions "title tag," "SEO title," or "optimize this page title." For meta descriptions, use meta-description-writer. For URL structure, use url-structure-optimizer.
metadata:
  version: 1.0.0
---
...
Title tag copy specifically. For meta descriptions, see **meta-description-writer**; for URL slugs, see **url-structure-optimizer**.
...
1. Confirm the primary target keyword for the page.
2. Draft a title placing the keyword naturally toward the front.
3. Add a differentiator or specificity element (number, year, unique angle) that gives a reason to click.
4. Check length against the ~50–60 character guideline, adjusting for pixel width if the title uses wide characters.
5. Add brand name at the end if space allows and it's not the primary driver of clicks.
6. Confirm the title doesn't duplicate another page's title exactly, which creates ambiguity about which page to rank for a given query.

## Related Skills

- **meta-description-writer**: Complementary SERP snippet element
- **url-structure-optimizer**: URL slug, ideally aligned with the title's core keyword
- **content-optimizer** (content): Broader on-page scoring this title feeds into
